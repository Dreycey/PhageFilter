...
impl ReadQueue {
    pub fn get_next_file(&mut self) {
        self.curr_file = self.filequeue.pop().map(|filepath| {
            let reader = BufReader::with_capacity(8192, File::open(&filepath).unwrap());
...
mod tests {
    ...
    fn test_get_lex_less() {
        ...
        let _revcomp_kmer = [b'A', b'C', b'G', b'T'];
        ...
        let _revcomp_kmer2 = [b'C', b'A', b'T', b'T'];
        ...
        let _revcomp_kmer3 = [b'C', b'T', b'A', b'C']; // <-- Lexically lesser
